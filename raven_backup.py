...
class RavenBackup:
    ...
    def __init__(self):
        ...
        self.i18n = gettext.translation(self.appname, self.localedir,
    ...
    def compilar_po_a_mo(self):
        ...
        if not os.path.exists(fichero_po_locales_en):
            ...
            if (not os.path.exists(fichero_mo_locales_en) or
                ...
                try:
                    subprocess.run([
                        "pybabel", "compile", "-i", fichero_po_locales_en, "-o", fichero_mo_locales_en]
                        , check=True, capture_output=True, text=True)
                except subprocess.CalledProcessError as e:
                    messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                        message=self._(f"Error al compilar el archivo de idioma {e.stderr.strip()}"))
                    quit()
    ...
    def crea_ventana_root(self):
        ...
        self.medida_ancho_ajustada = round(self.ancho_pantalla / 2 - self.ancho_ventana / 2)
        self.medida_alto_ajustada = round(self.alto_pantalla / 2 - self.alto_ventana / 2)
        self.root.geometry(str(self.ancho_ventana) + "x" + str(self.alto_ventana)
                           + "+" + str(self.medida_ancho_ajustada)
                           + "+" + str(self.medida_alto_ajustada))
        self.root.resizable(False, False)
        self.root.iconphoto(True, PhotoImage(file="assets/raven_backup.png"))  # True pone icono en mensajes
        self.root.title(self.nombre_aplicacion + ' ' + 'v.' + self.version +
                        15 * ' ' + self._('Configuración actual: ') + self.chequea_fichero_conf_actual().upper())
    ...
    def chequea_fichero_conf_actual(self):
        ...
        for fichero in glob.glob("*.conf"):
            ...
            if len(sp.stdout) == 0:
                self.nombre_fichero_configuracion_actual = fichero.split(".")[0]
    ...
    def establece_idioma(self,idioma):
        ...
        self.i18n = gettext.translation(self.appname, self.localedir, fallback=True, languages=[self.idioma_seleccionado])
        self._ = self.i18n.gettext
    ...
    def acerca_de(self):
        ...
        acerca_de_win.title(self.nombre_aplicacion + ' ' + 'v.' + self.version)
        lbl_acerca_de_win = Label(master=acerca_de_win)
        lbl_acerca_de_win.configure(text=self._('Raven Backup es un script en python'
        ...
        acerca_de_win.after(tiempo_espera_destruir, lambda: acerca_de_win.destroy())
    ...
    def recoge_valores_cron(self):
        ...
        if not self.cron_arroba_var.get() == '' and \
            ...
            messagebox.showinfo(title=self.nombre_aplicacion + ' ' +
                                      'v.' + self.version,
                                message=self._('Seleccione sólo un tipo de parámetro temporal'))
        ...
        self.texto_etiqueta_mensaje_de_estado_cron.set(self._('Construyendo instrucciones para cron...'))
        ...
        for elemento in self.comandos_para_cron_lst:
            ...
            item_cron = '"' + item_cron + '"'
            comando = "(crontab -l 2>/dev/null; echo " + item_cron + ") | crontab -"
            ...
            for elemento_lista_chequeo in chequea_resultado_lst:
                if elemento_lista_chequeo == item_cron.replace('"', ""):
                    ...
                    self.cron_ventana.wm_attributes('-topmost', False)
                    messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                        message=self._('Ya existe en cron el elemento: ')
                                                + signos_temporales + ' ' + item_origen[-2]
                                                + self._('\nSe omite'))
                    self.cron_ventana.wm_attributes('-topmost', True)
                    cont = True
                else:
    ...
    def selec_idioma_para_config(self, ancho=350, alto=140):
        ...
        combo.current(1) # Inglés
        aceptar_ventana_msg_btn = Button(selec_idioma_ventana, text=self._('Aceptar'),
            command=lambda: [self.establece_idioma(combo.get().split("|")[1]),
                             selec_idioma_ventana.destroy()], font=("TkDefaultFont", 12))
    ...
    def programador_cron(self):
        ...
        parametros_temporales_cron_lf = LabelFrame(self.cron_ventana, text=self._('Parámetros de tiempo')
                                                   , width=530, height=90, font=("TkDefaultFont", 12))
        ...
        self.cron_ventana.title(self.nombre_aplicacion + ' ' + 'v.' + self.version)
        cron_minuto_lbl = Label(parametros_temporales_cron_lf, text=self._('Minuto'),  font=("TkDefaultFont", 12))
        cron_minuto_lbl.grid(row=0, column=0)
        cron_minuto_combobox = ttk.Combobox(parametros_temporales_cron_lf, textvariable=self.cron_minuto_var, width=5)
        cron_minuto_combobox['values'] = [m for m in range(0, 60)]
        cron_minuto_combobox.grid(row=1, column=0, padx=10, pady=10)
        cron_hora_lbl = Label(parametros_temporales_cron_lf, text=self._('Hora'), font=("TkDefaultFont", 12))
        cron_hora_lbl.grid(row=0, column=1)
        cron_hora_combobox = ttk.Combobox(parametros_temporales_cron_lf, textvariable=self.cron_hora_var, width=5)
        cron_hora_combobox['values'] = [m for m in range(0, 24)]
        cron_hora_combobox.grid(row=1, column=1, padx=0, pady=0)
        cron_dia_mes_lbl = Label(parametros_temporales_cron_lf, text=self._('Día del mes'), font=("TkDefaultFont", 12))
        cron_dia_mes_lbl.grid(row=0, column=2, padx=10, pady=0)
        cron_hora_combobox = ttk.Combobox(parametros_temporales_cron_lf, textvariable=self.cron_dia_mes_var, width=5)
        cron_hora_combobox['values'] = [m for m in range(1, 32)]
        cron_hora_combobox.grid(row=1, column=2, padx=10, pady=0)
        cron_mes_lbl = Label(parametros_temporales_cron_lf, text=self._('Mes'), font=("TkDefaultFont", 12))
        cron_mes_lbl.grid(row=0, column=3, padx=10, pady=0)
        cron_mes_combobox = ttk.Combobox(parametros_temporales_cron_lf, textvariable=self.cron_mes_var, width=5)
        cron_mes_combobox['values'] = [month_name[m][0:3] for m in range(1, 13)]
        cron_mes_combobox.grid(row=1, column=3, padx=10, pady=0)
        cron_dia_semana_lbl = Label(parametros_temporales_cron_lf, text=self._('Día de la semana'), font=("TkDefaultFont", 12))
        cron_dia_semana_lbl.grid(row=0, column=4, padx=10, pady=0)
        cron_dia_semana_combobox = ttk.Combobox(parametros_temporales_cron_lf, textvariable=self.cron_dia_semana_var, width=5)
        ...
        cron_arroba_lbl = Label(self.cron_ventana, text=self._('Cadenas de texto reservadas'), font=("TkDefaultFont", 12))
        cron_arroba_lbl.place(x=25, y=129)
        cron_arroba_combobox = ttk.Combobox(self.cron_ventana, textvariable=self.cron_arroba_var, width=9)
        ...
        Label(self.cron_ventana, text=self._('Línea de mensajes:'), font=("TkDefaultFont", 12)).place(x=25, y=210)
        ...
        Label(self.cron_ventana, text=self._('Puede editar los desplegables manualmente'),
              font=("TkDefaultFont", 9)).place(x=x_pos, y=102)
        aceptar_cron_btn = Button(self.cron_ventana, text=self._('Aceptar'), command=self.recoge_valores_cron, font=("TkDefaultFont", 12))
        aceptar_cron_btn.place(x=170, y=172)
        cerrar_cron_btn = Button(self.cron_ventana, text=self._('Cerrar'),
                                 command=lambda: [self.cron_ventana.destroy(),self.root.focus()], font=("TkDefaultFont", 12))
        cerrar_cron_btn.place(x=295, y=172)
        mensaje_de_estado_lbl_cron = Label(self.cron_ventana, textvariable=self.texto_etiqueta_mensaje_de_estado_cron,
    ...
    def crea_menus(self):
        ...
        menu_opciones_idiomas = Menu(menubar, tearoff=0)  # tear-off = 0 no deja que una ventana sea flotante
        menu_opciones_idiomas.add_command(label=self._('Español'), command=lambda: self.selecciona_idioma('es'))
        menu_opciones_idiomas.add_command(label=self._('Inglés'), command=lambda: self.selecciona_idioma('en'))
        menubar.add_cascade(label=self._('Opciones de idioma'), menu=menu_opciones_idiomas)
        menubar.add_command(label=self._('Programador'), command=lambda: self.programador_cron())
        menubar.add_command(label=self._('Configuraciones'), command=lambda: self.cambiar_configuracion())
        menubar.add_command(label=self._('Acerca de'), command=lambda: self.acerca_de())
    ...
    def lee_fichero_configuracion(self):
        ...
        if os.path.isfile('.raven_backup.conf'):
            ...
            if not self.destino.find(':') == -1:
                ...
                self.password = opciones_dict.get('password')
                self.password_var_inicial_str = opciones_dict.get('password')
            else:
            ...
            self.opcion_comprimir_var.set(opciones_dict.get('comprimir_durante_copia'))
            ...
            self.opcion_enlaces_simbolicos_var.set(opciones_dict.get('tratamiento_archivos_enlazados'))
            self.opcion_enlaces_simbolicos_inicial_str = self.opcion_enlaces_simbolicos_var.get()
            opciones_dict = ast.literal_eval(items_fichero_configuracion[5])
            self.opcion_incremental_var.set(opciones_dict.get('copia_incremental'))
            self.opcion_incremental_inicial_str = self.opcion_incremental_var.get()
    ...
    def cuenta_atras(self, contador):
        ...
        if contador == 3: # TODO: Algo raro en los mensajes
            if not self.texto_etiqueta_mensaje_de_estado.get() == 'Copia finalizada' or \
                    not self.texto_etiqueta_mensaje_de_estado.get() == 'Backup done':
    ...
    def procesa_datos_origen(self, dry=False):
        ...
        if self.directorios_seleccionados_lstbox.size() == 0:
            if dry:
                self.cron_ventana.wm_attributes('-topmost', False)
            messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                message=self._('No hay ficheros y/o directorios seleccionados para copiar'))
            if dry:
                self.cron_ventana.wm_attributes('-topmost', True)
                self.texto_etiqueta_mensaje_de_estado_cron.set('')
            return None
        ...
        if len(self.directorios_seleccionados_lstbox.curselection()) != 0:
            ...
            for i in range(len(self.directorios_seleccionados_lstbox.curselection())):
                indice = self.directorios_seleccionados_lstbox.curselection()[i]
                items_seleccionados += self.directorios_seleccionados_lstbox.get(indice, indice)
            directorios_seleccionados_tupla = items_seleccionados
        else:
            directorios_seleccionados_tupla = self.directorios_seleccionados_lstbox.get('0', 'end')

        directorio_destino = self.directorio_destino_txtbox.get('1.0', 'end').strip()
        ...
        if not directorio_destino.find(':') == -1:
            if self.puerto_remoto_txtbox.get('1.0', 'end').strip() == '':
                if dry:
                    self.cron_ventana.wm_attributes('-topmost', False)
                messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                    message=self._('Debe asignar un valor de puerto remoto para un destino remoto'))
                if dry:
                    self.cron_ventana.wm_attributes('-topmost', True)
                    self.texto_etiqueta_mensaje_de_estado_cron.set('')
                return None
        ...
        if not dry:
            self.texto_etiqueta_mensaje_de_estado.set(self._('Chequeando algunas cosas...'))

        indice = 0
        for item_a_copiar in directorios_seleccionados_tupla:
            if self.directorios_seleccionados_lstbox.size() > indice:
                if not dry:
                    self.directorios_seleccionados_lstbox.itemconfig(indice, bg="lightgrey")
                ...
                if os.path.isdir(item_a_copiar):
                    ...
                    if self.opcion_incremental_var.get() == 'True':
                        if dry:
                            ...
                            self.log_txtbox.insert(tkinter.INSERT, self._('Se crean enlaces simbólicos para cron: ' + '\n'))
                            self.log_txtbox.update()
                        fichero_simbolico_a_chequear = item_a_copiar_cadena_final + '_latest'
                        ruta_fichero_simbolico_a_chequear = directorio_destino + '/' + fichero_simbolico_a_chequear
                        chequea_enlace_simbolico_cmd = ['if [ -L  ' + ruta_fichero_simbolico_a_chequear + ' ]; then echo Existe enlace  ; fi']
                        result = subprocess.run(chequea_enlace_simbolico_cmd, capture_output=True, text=True, shell=True)
                        if not result.stderr == '':
                            if dry:
                                self.cron_ventana.wm_attributes('-topmost', False)
                                messagebox.showerror(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                                 message=self._('Error verificando enlaces simbólicos en destino'))
                                self.cron_ventana.wm_attributes('-topmost', True)
                                self.texto_etiqueta_mensaje_de_estado_cron.set('')
                        else:
                            self.log_txtbox.insert(tkinter.INSERT, '\n')
                            self.log_txtbox.insert(tkinter.INSERT, self._('Chequeando enlace ') + item_a_copiar
                                                   + ' ---> ' + ruta_fichero_simbolico_a_chequear + '\n')
                            self.log_txtbox.update()
                        if "Existe enlace" in result.stdout:
                            self.log_txtbox.insert(tkinter.INSERT, '\n')
                            self.log_txtbox.insert(tkinter.INSERT, self._('Existe enlace ') + item_a_copiar
                                                   + ' ---> ' + ruta_fichero_simbolico_a_chequear + '\n')
                            self.log_txtbox.update()
                        else:
                            ruta_directorio_origen = directorio_destino + '/' + item_a_copiar_cadena_final
                            if not os.path.isdir(directorio_destino_ruta_completa):
                                result = subprocess.run(['mkdir -p ' + directorio_destino_ruta_completa], text=True, capture_output=True, shell=True)
                                if not result.stderr == '':
                                    if dry:
                                        self.cron_ventana.wm_attributes('-topmost', False)
                                    messagebox.showerror(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
            ...
            if result.stderr != '':
                ...
                if dry:
                    self.cron_ventana.wm_attributes('-topmost', False)
                messagebox.showerror(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                     message=self._('Error en el proceso Abandonando la copia'))
                if dry:
                    self.cron_ventana.wm_attributes('-topmost', True)
                    self.texto_etiqueta_mensaje_de_estado_cron.set('')
                return None
    ...
    def guarda_lista_items_seleccionados(self):
        ...
        if not self.directorio_destino_txtbox.get('1.0', 'end').strip().find(':') == -1:
            ...
            if self.tipo_acceso_var.get() == 'password' and self.password_var.get() == '':
                messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                    message=self._('Debe asignar un valor de password para acceso con password'))
                return None
        if not (self.modificacion_de_items or not self.opcion_delete_inicial_str == self.opcion_delete_var.get() or
                not self.opcion_comprimir_inicial_str == self.opcion_comprimir_var.get() or
        not self.opcion_enlaces_simbolicos_inicial_str == self.opcion_enlaces_simbolicos_var.get() or
            not self.opcion_incremental_inicial_str == self.opcion_incremental_var.get()):
            # Presenta un mensaje en una label sobre la pantalla.
            self.texto_etiqueta_mensaje_de_estado.set(self._('No hay cambios que guardar'))
            self.cuenta_atras(0)
            return None
        self.comprueba_existencia_en_origen()
        directorios_seleccionados_tupla = self.directorios_seleccionados_lstbox.get('0', 'end')
        ...
        if len(directorios_seleccionados_tupla) == 0 or len(directorio_destino) == 0:
            if len(directorios_seleccionados_tupla) == 0:
                messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                    message=self._('No existen ficheros o directorios de origen para guardar'))
            if len(directorio_destino) == 0:
                messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                    message=self._('No existe directorio de destino para guardar'))
            return None

        file = filedialog.asksaveasfile(
            filetypes=(("Configuration files", "*.conf"), ("Configuration files", "*.conf")),
        ...
        _dict = "{'comprimir_durante_copia':" + "'" + self.opcion_comprimir_var.get() + "'" + "}"

        fichero_configuracion.write(_dict)
        fichero_configuracion.write('\n')

        if self.opcion_enlaces_simbolicos_var.get() == '':
            self.opcion_enlaces_simbolicos_var.set('copiar_archivos_enlazados')
        _dict = "{'tratamiento_archivos_enlazados':" + "'" + self.opcion_enlaces_simbolicos_var.get() + "'" + "}"
        ...
        _dict = "{'copia_incremental':" + "'" + self.opcion_incremental_var.get() + "'" + "}"
        ...
        self.opcion_enlaces_simbolicos_inicial_str = self.opcion_enlaces_simbolicos_var.get()
        self.opcion_incremental_inicial_str = self.opcion_incremental_var.get()
        self.directorio_destino_inicial_str = self.directorio_destino_txtbox.get('1.0', 'end').strip()
        self.puerto_remoto_inicial_str = self.puerto_remoto_txtbox.get('1.0', 'end').strip()
    ...
    def cambiar_configuracion(self):
        ...
        fichero_temporal = (filedialog.askopenfilename(filetypes=[("Configuration files", "*.conf")],
        ...
        if f_a_comparar == self.nombre_fichero_configuracion_actual.strip():
            messagebox.showinfo(title=self._('Información'),
                                message=self._('Su elección ya es la configuración actual'))
            return
        respuesta = askyesno(title=self._('Confirmación'),
        ...
        if respuesta:
            ...
            self.texto_etiqueta_mensaje_de_estado.set(
                self._('El fichero ') + fichero_temporal + self._(' es ahora el fichero de parámetros'))
            self.cuenta_atras(0)
            self.reinicia_gui()
        ...
        for item_ in directorios_seleccionados_tupla:
            ...
            if not os.path.exists(item_):
                ...
                if os.path.exists(directorio_destino + '/' + item_):
                    respuesta = askyesno(title=self._('Confirmación'),
                                         message=self._('El item de origen ')
                                                 + item_ + self._(' existe en destino\n')
                                                 + self._('¿Desea eliminar en destino?'))
                    if respuesta:
    ...
    def selecciona_directorios_origen(self):
        ...
        for tmp in range(self.directorios_seleccionados_lstbox.size()):
            if directorio_origen == self.directorios_seleccionados_lstbox.get(tmp):
    ...
    def selecciona_ficheros_origen(self):
        ...
        for tmp in range(self.directorios_seleccionados_lstbox.size()):
            if fichero_origen == self.directorios_seleccionados_lstbox.get(tmp):
    ...
    def comprueba_cambios_y_salir(self):
        ...
        if (self.modificacion_de_items or self.modificacion_checkboxes
            ...
            respuesta = askyesno(title=self._('Confirmación'),
                                 message=self._('Se han realizado modificaciones\n') + self._('¿Guardar los cambios antes de salir?'))
            if respuesta:
    ...
    def chequea_seleccion_previa_item(self, event):
        ...
        if len(self.directorios_seleccionados_lstbox.curselection()) != 0:
            if self.item_seleccionado == self.directorios_seleccionados_lstbox.curselection():
                self.directorios_seleccionados_lstbox.selection_clear(0, 'end')
        self.item_seleccionado = self.directorios_seleccionados_lstbox.curselection()
    ...
    def chequea_modificacion_destino(self, event):
        ...
        if self.directorio_destino_inicial_str == self.directorio_destino_txtbox.get('1.0', 'end').strip():
            self.modificacion_de_items = False
        else:
            self.modificacion_de_items = True
    ...
    def chequea_comprimir_durante_copia(self):
        ...
        if self.opcion_comprimir_var.get() == self.opcion_comprimir_inicial_str:
            self.modificacion_checkboxes = False
        else:
    ...
    def chequea_modificacion_copia_incremental(self, event):
        ...
        if self.opcion_incremental_var.get() == 'True':
            messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                            message=self._('Advertencia: rsync no admite copia incremental para ficheros') + '\n'
                                      + self._('Si lo desea cree una carpeta e introduzca los ficheros') + '\n'
                                      + self._('en ella, después seleccione esta carpeta para copiar'))
    ...
    def chequea_modificacion_puerto_remoto(self, event):
        ...
        if self.directorio_destino_txtbox.get('1.0', 'end').find(':') == -1:
            messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                message=self._('No se guardarán parámetros remotos para copia local'))
    ...
    def chequea_modificacion_password(self, event):
        ...
        if self.tipo_acceso_var.get() == 'clave_publica':
            messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                message=self._('No se guardará contraseña para opción de accceso con clave pública'))
            self.password_var.set('')
            self.root.focus()
            return None
    ...
    def chequea_modificacion_tipo_acceso(self, event=None):
        ...
        if self.tipo_acceso_var.get() == 'clave_publica' and self.password_var.get() != '':
            messagebox.showinfo(title=self.nombre_aplicacion + ' ' + 'v.' + self.version,
                                message=self._('No se guardará contraseña para opción de accceso con clave pública'))
            self.password_var.set('')
            self.modificacion_de_items = True
    ...
    def rellena_datos(self):
        ...
        if not self.destino.find(':') == -1:
            self.puerto_remoto_txtbox.insert(tkinter.INSERT, self.puerto_remoto)
            self.password_entrybox.insert(0, self.password)
        if not len(self.origen_lst) == 0:
            for item in range(len(self.origen_lst)):
                self.directorios_seleccionados_lstbox.insert('end', self.origen_lst[item].strip())
        if not len(self.destino) == 0:
            if self.destino.find(':') == -1:
                self.directorio_destino_txtbox.insert(tkinter.INSERT, self.destino)
            else:
                self.directorio_destino_txtbox.insert(tkinter.INSERT, self.destino_ip)
    ...
    def chequea_modificacion_enlaces_simbolicos(self):
        ...
        if self.opcion_enlaces_simbolicos_var.get() == self.opcion_enlaces_simbolicos_inicial_str:
            self.modificacion_radiobotones_ficheros_con_enlaces = False
        else:
    ...
    def crea_elementos_graficos(self):
        ...
        opciones_remoto_lf = LabelFrame(self.root,  text=self._('Opciones Remoto'), width=370, height=75, font=("TkDefaultFont", 12))
        opciones_remoto_lf.place(x=810, y=95)
        tipo_acceso_lf = LabelFrame(self.root,  text=self._('Tipo de Acceso Remoto'), width=288, height=60, font=("TkDefaultFont", 12))
        tipo_acceso_lf.place(x=490, y=100)
        opciones_ficheros_enlazados_lf = LabelFrame(self.root,  text=self._('Opciones para ficheros enlazados'), width=350, height=90, font=("TkDefaultFont", 12))
        ...
        self.password_radiobtn.bind('<ButtonRelease>', self.chequea_modificacion_tipo_acceso)
        ...
        Label(self.root, text=self._('Directorios y ficheros de origen seleccionados'),
              font=("TkDefaultFont", 12)).place(x=35, y=25)
        Label(self.root, text=self._('Destino seleccionado (Local/Remoto)'), font=("TkDefaultFont", 12)).place(x=490, y=25)
        Label(self.root, text=self._('Log del proceso'), font=("TkDefaultFont", 12)).place(x=490, y=180)
        Label(self.root, text=self._('Línea de mensajes:'), font=("TkDefaultFont", 12)).place(x=37, y=515)
        Label(opciones_remoto_lf, text=self._('Puerto Remoto'), font=("TkDefaultFont", 11)).place(x=10, y=10)
        Label(opciones_remoto_lf, text=self._('Password'), font=("TkDefaultFont", 11)).place(x=192, y=10)
        mensaje_de_estado_lbl = Label(self.root, textvariable=self.texto_etiqueta_mensaje_de_estado,
                                      font=(font.Font(None, size=11, weight="bold")))
        ...
        self.directorios_seleccionados_lstbox = Listbox(self.dir_selected_lstbox_frame, width=40, height=6, font=("TkDefaultFont", 12), selectmode='multiple')
        self.directorios_seleccionados_lstbox.grid(row=0, column=0, sticky=tkinter.EW)
        self.directorios_seleccionados_lstbox.bind('<<ListboxSelect>>', self.chequea_seleccion_previa_item)

        self.directorio_destino_txtbox = Text(self.root, height=1, width=39, font=("TkDefaultFont", 12))
        self.directorio_destino_txtbox.place(x=490, y=50)
        self.directorio_destino_txtbox.bind('<KeyRelease>', self.chequea_modificacion_destino)

        self.puerto_remoto_txtbox = Text(opciones_remoto_lf, height=1, width=5, font=("TkDefaultFont", 11))
        self.puerto_remoto_txtbox.place(x=128, y=8)
        self.puerto_remoto_txtbox.bind('<KeyRelease>', self.chequea_modificacion_puerto_remoto)

        self.password_entrybox = Entry(opciones_remoto_lf, width=8, show='*', textvariable=self.password_var, font=("TkDefaultFont", 11))
        self.password_entrybox.place(x=273, y=8)
        self.password_entrybox.bind('<KeyRelease>', self.chequea_modificacion_password)

        self.log_txtbox = Text(self.log_txtbox_frame, height=11, width=73, font=("TkDefaultFont", 12))
        self.log_txtbox.grid(row=0, column=0, sticky=tkinter.EW)

        scrollbar_0 = Scrollbar(self.dir_selected_lstbox_frame, orient='vertical',
                                command=self.directorios_seleccionados_lstbox.yview)
        scrollbar_0.grid(row=0, column=1, sticky=tkinter.NS)
        self.directorios_seleccionados_lstbox['yscrollcommand'] = scrollbar_0.set
        scrollbar = Scrollbar(self.log_txtbox_frame, orient='vertical', width=16, command=self.log_txtbox.yview)
        scrollbar.grid(row=0, column=1, sticky=tkinter.NS)
        self.log_txtbox['yscrollcommand'] = scrollbar.set

        self.option_delete_chkbtn = Checkbutton(self.root,
                                           text=self._(' Eliminar en destino si ha sido borrado en origen'),
                                           font=("TkDefaultFont", 12),

                                           onvalue='True',
                                           offvalue='False',
            command= lambda: [self.chequea_eliminar_en_destino(), self.cambia_color_label_delete()])
        self.option_delete_chkbtn.place(x=30, y=332)
        self.option_comprimir_chkbtn = Checkbutton(self.root,
        ...
        self.option_incremental_chkbtn = Checkbutton(self.root,
                                                text=self._('Copia incremental'),
                                                font=("TkDefaultFont", 12),
                                                onvalue='True',
                                                offvalue='False')
        self.option_incremental_chkbtn.place(x=30, y=383)
        self.option_incremental_chkbtn.bind('<ButtonRelease>', self.chequea_modificacion_copia_incremental)

        self.cambia_color_label_delete()

        (Button(self.left_buttons_frame, text=self._('Elegir ficheros'), command=self.selecciona_ficheros_origen, font=("TkDefaultFont", 12))
         .grid(row=0, column=0, padx=3, pady=3))
        (Button(self.left_buttons_frame, text=self._('Elegir directorios'), command=self.selecciona_directorios_origen, font=("TkDefaultFont", 12))
         .grid(row=0, column=1, padx=3, pady=3))
        (Button(self.left_buttons_frame, text=self._('Eliminar'), command=self.elimina_items_seleccionados, font=("TkDefaultFont", 12))
         .grid(row=0, column=3, padx=3, pady=3))
        Button(self.root, text=self._('Elegir destino local'), command=self.selecciona_destino, font=("TkDefaultFont", 12)).place(x=910, y=45)
        (Button(self.left_buttons_frame,  text=self._('Guardar'), width=11, command=self.guarda_lista_items_seleccionados, font=("TkDefaultFont", 12))
         .grid(row=1, column=0, padx=3, pady=3))
        (Button(self.left_buttons_frame, text=self._(' Iniciar copia '), width=14, command=self.lanzar_hilo_copia, font=("TkDefaultFont", 12))
         .grid(row=1, column=1, padx=3, pady=3))
        (Button(self.left_buttons_frame, text=self._('Salir'), width=6, command=self.comprueba_cambios_y_salir, font=("TkDefaultFont", 12))
         .grid(row=1, column=3, padx=3, pady=3))
        (Button(self.log_txtbox_frame, text=self._('Limpia Log'), width=8, command=self.limpia_log, font=("TkDefaultFont", 12))
